...
**Regra de integração (feedback do Luís, 08/08 — reincidência é inaceitável):** nenhum bloco fecha olhando só para o próprio pedaço. Antes de entregar, faça a **varredura de integração ponta a ponta**: quem CONSOME o que este bloco produz e quem PRODUZ o que ele consome (funil → análise → formalização → cobrança → carteira; catálogo → simulação → contrato; aprovações → efetivadores). Se outro módulo já tem estrutura esperando o dado (enum, estado, campo, critério de motor), o bloco novo se INTEGRA a ela — nunca cria um campo paralelo. Divergência descoberta na varredura é sinalizada e resolvida ANTES do commit. **E funcionalidade sem tela não é entregável**: todo dado ou ação nova do backend nasce com sua representação em tela no MESMO bloco, com nome e propósito compreensíveis pelo operador (segundo feedback do Luís sobre isso — não há terceiro).
