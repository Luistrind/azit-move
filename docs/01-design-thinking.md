...
### ItemContratado
Cada produto dentro da cesta do contrato de crédito. Pode ser recorrente (proteção veicular, taxa de serviço, rastreador) ou parcelado (financiamento do veículo, crédito avulso). Tem origem **venda** ou **renegociacao**. Cada item tem um **destino de recebível** definido — quem é o credor real daquele valor.
...
O cliente comprou o veículo. A obrigação financeira persiste independente do que acontecer com o bem — sinistro, furto, dano. Parcelas não são perdoadas, são renegociadas ou repactuadas.
...
Ações críticas como desbloqueio de veículo, renegociação e repactuação são registradas por humanos no sistema. O sistema executa as consequências automaticamente.
...
1. O contrato e os dados do titular chegam via API do PopHub no momento da assinatura
2. O sistema identifica ou cria o titular (pelo CPF/CNPJ) e sua conta; identifica o ativo e vincula automaticamente
3. A entrada, se houver, já foi paga no momento da originação — o sistema registra e concilia
4. O sistema gera o cronograma completo de recebíveis a partir do saldo devedor pós-entrada
5. O sistema cria todas as faturas futuras com seus itens previstos
6. O contrato entra em status ativo
...
### Fluxo 4 — Renegociação

**Renegociação menor (novação)**
Titular tem parcelas atrasadas. O Arthur registra a renegociação no sistema:
- Parcelas antigas são marcadas como **renegociadas** (extintas)
- O acordo gera um **ItemContratado novo de origem RENEGOCIACAO** — um crédito que substitui as parcelas extintas, no modelo bancário de novação
...
**Repactuação radical**
Situação grave que exige recomeço:
- Contrato antigo é liquidado
- Novo contrato nasce com novas condições
- Começa uma nova história do zero, com registro da baixa do contrato anterior

A renegociação é sempre uma **operação humana registrada no sistema** pelo time operacional, sujeita à estrutura de alçadas. O sistema executa as consequências automaticamente após a confirmação da entrada via webhook.
...
A dívida não é perdoada pelo sinistro. Parcelas não pagas são renegociadas ou repactuadas, nunca apagadas.
...
- ⏳ **Aprimoramento dos contratos** — discriminar melhor os itens nos próximos contratos do PopHub

---

*Documento gerado em 2026-06-23 como exercício de design thinking do modelo futuro (to-be) da plataforma Azit Move V3.*
